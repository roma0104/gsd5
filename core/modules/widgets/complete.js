...
CompleteWidget.prototype.getValue = function() {
	...
	if(tiddler.fields.gtd_complete === "true") {
		return true;
	}
	return false;
};
...
CompleteWidget.prototype.handleChangeEvent = function() {
	this.wiki.setText(this.completeTitle, "gtd_complete", null, this.inputDomNode.checked.toString());
	var now = new Date();
	this.wiki.setText(this.completeTitle, "gtd_comp_date", null, $tw.utils.stringifyDate(now));
...
CompleteWidget.prototype.changeDependants = function() {
	...
	if(this.inputDomNode.checked === true) {
		this.wiki.forEachTiddler(function(title, tiddler) {
			if(tiddler.fields.gtd_status === "future" && tiddler.fields.gtd_prereq === self.completeTitle) {
				this.setText(title, "gtd_status", null, "next");
			}
		});
	}
};
